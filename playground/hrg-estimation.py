...
def calculate(A):
    ...
    if args_rsmooth < 2:
        print("r_smooth should be greater than 1.")
    if args_alphakernel <= 0.5:
        print("alpha of kernel estimator should be grater than 0.5.")
    if args_hsteps <= 0:
        print("hsteps should be greater than 0.")
    if args_noise != 0 and args_noise != 1:
        print("noise flag should be 0 or 1.")
    if args_pnoise < 0:
        print("pnoise parameter should be greater or equal to 0.")
    if args_bootstrap != 0 and args_bootstrap != 1:
        print("bootstrap flag should be 0 or 1.")
    if args_tbootstrap <= 0.0 or args_tbootstrap >= 1.0:
        print("tbootstrap should be in range (0, 1).")
    if args_rbootstrap <= 0:
        print("Number of bootstrap resamples should be greater than 0.")
    if args_amseborder <= 0.0:
        print("amseborder should be greater than 0.")
    if args_diagplots != 0 and args_diagplots != 1:
        print("diagplots flag should be 0 or 1.")
    if args_verbose != 0 and args_verbose != 1:
        print("verbose flag should be 0 or 1.")
    if args_savedata != 0 and args_savedata != 1:
        print("savedata flag should be 0 or 1.")
    if args_theta1 < 0.0 or args_theta1 > 1.0:
        print("Theta parameters should be in [0,1] range, where theta1 < theta2.")
    if args_theta2 < 0.0 or args_theta2 > 1.0:
        print("Theta parameters should be in [0,1] range, where theta1 < theta2.")
    if args_theta2 <= args_theta1:
        print("Theta parameters should be in [0,1] range, where theta1 < theta2.")
    if args_delimiter not in set(['whitespace', 'tab', 'comma', 'semicolon']):
        print("Delimiter provided is not supported.")

    number_of_bins = args_nbins
    r_smooth = args_rsmooth
    alpha = args_alphakernel
    hsteps = args_hsteps
    if args_noise == 1:
        noise_flag = True
    else:
    ...
    t_bootstrap = args_tbootstrap
    r_bootstrap = args_rbootstrap
    amse_border = args_amseborder
    if args_diagplots == 1:
    ...
    if args_delimiter == "whitespace":
        delimiter = " "
    elif args_delimiter == "tab":
        delimiter = "\t"
    elif args_delimiter == "comma":
        delimiter = ","
    elif args_delimiter == "semicolon":
        delimiter = ";"

    # check for number of entries
    N = len(A)
    ordered_data = A.sum(axis=0)
    ordered_data.sort()
    ...
    print("========================")
    make_plots(ordered_data, number_of_bins,
               r_smooth, alpha, hsteps, bootstrap_flag, t_bootstrap,
               r_bootstrap, diagnostic_plots_flag, eps_stop, 
               args_theta1, args_theta2, verbose, noise_flag,
               p_noise)
