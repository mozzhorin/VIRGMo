'''
Von Mises distribution for Pytorch from 
...
class VonMisesFisher(torch.distributions.Distribution):
    ...
    def __init__(self, loc, scale, validate_args=None):
        ...
        self.__e1 = (torch.Tensor([1.] + [0] * (loc.shape[-1] - 1))).to(self.device).to(self.dtype)
    ...
    def rsample(self, shape=torch.Size()):
        ...
        v = (torch.distributions.Normal(0, 1).sample(
            shape + torch.Size(self.loc.shape)).to(self.device).to(self.dtype).transpose(0, -1)[1:]).transpose(0, -1)
    ...
    def __sample_w3(self, shape):
        ...
        u = torch.distributions.Uniform(0, 1).sample(shape).to(self.device).to(self.dtype)
    ...
    def __sample_w_rej(self, shape):
        ...
        s = torch.min(torch.max(torch.tensor([0.], device=self.device, dtype=self.dtype),
                                self.scale - 10), torch.tensor([1.], device=self.device, dtype=self.dtype))
    ...
    def __while_loop(self, b, a, d, shape):
        ...
        while bool_mask.sum() != 0:
            e_ = torch.distributions.Beta((self.__m - 1) / 2, (self.__m - 1) /
                                          2).sample(shape[:-1]).reshape(shape).to(self.device).to(self.dtype)
            u = torch.distributions.Uniform(0, 1).sample(shape).to(self.device).to(self.dtype)
...
#             raise RuntimeError('v must be >= 0, it is {}'.format(v))
        ...
        return torch.Tensor(output).to(z.device).to(z.dtype)
