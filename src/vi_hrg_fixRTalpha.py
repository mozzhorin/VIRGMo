...
class VI_HRG(torch.nn.Module):
    ...
    def params_reset(self):
        ...
        self.R_f = Variable(R_conc, requires_grad=False).to(self.device).to(self.dtype)
        self.T_f = Variable(T[0], requires_grad=False).to(self.device).to(self.dtype)
        self.a_f = Variable(alpha_conc, requires_grad=False).to(self.device).to(self.dtype)
...
#        return torch.mul(log_pA, temp).sum()
    ...
    def elbo(self, idx1, idx2, weights, debug=False):
        ...
        r_x_loc, r_x_scale, phi_x_loc, phi_x_scale, R_x, T_x, \
            alpha_x = self.constrained_params() 
...
#        alpha_samples = alpha_q.rsample([self.num_samples]).squeeze(-1).to(self.device).to(self.dtype)
        ...
        r_q = Radius(r_x_loc, 
                       r_x_scale, 
                       R_x.expand(self.num_nodes))
        r_samples = r_q.rsample([self.num_samples]).to(self.device).to(self.dtype)
        
        alpha_r_i = log1mexp(alpha_x.expand(L,self.num_samples).t()*r_samples[:,idx1]*2)
        alpha_R = log1mexp(alpha_x*R_x)
        alpha_R_ri = alpha_x.expand(L,self.num_samples).t()*\
                    (R_x.expand(L,self.num_samples).t()-r_samples[:,idx1])
...
#        
        ...
        p_ = phd(warn_tensor(dist,'dist'), R_x.expand(L,self.num_samples).t(), T_x.expand(L,self.num_samples).t())
...
#        if debug: print('-D_kl(T)    >>', str(elbo3)) 
        ...
        elbo6 = L/self.num_nodes * alpha_x.log().mean()
...
#        if debug: print('Log(2*pi)   >>', str(elbo7))
        ...
        elbo9 = - 1/self.num_nodes * r_q.log_prob(r_samples)[:,idx1].mean(dim=0).sum()
        if debug: print('P(q_ri)     >>', str(elbo9))
        elbo10 = -1/self.num_nodes * phi_q.log_prob(phi_samples)[:,idx1].mean(dim=0).sum()
        if debug: print('P(q_phii)   >>', str(elbo10))
        return elbo4+elbo5+elbo6+elbo7+elbo8+elbo9+elbo10 
