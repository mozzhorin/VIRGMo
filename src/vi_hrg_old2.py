...
class VI_HRG(torch.nn.Module):
    ...
    def __init__(self, num_nodes=50, num_samples=20, 
        ...
        if priors['T_p'] is None:
            T_p = torch.tensor([1., 10.]).to(self.device).to(self.dtype)
        else:
            T_p = priors['T_p'].to(self.device).to(self.dtype)
        
        if priors['alpha_p'] is None:
            alpha_p = torch.tensor([2., 0.5]).to(self.device).to(self.dtype)
        else:
...
#        dist = arcosh_(warn_tensor(cosh_dist,'cosh_dist')) #+self.epsilon
        ...
        elbo5 = L/self.num_nodes**magic_power * (a_R_ri+l1e_a_ri).mean(dim=0).sum()
#        print(a_R_ri)
#        print(l1e_a_ri)
...
#        print(R_samples)
        elbo9 = - L/self.num_nodes**magic_power * r_q_lp[:,idx1].mean(dim=0).sum()
        if debug: print('P(r_q)     >>', str(elbo9))
        q_phi_entropy = phi_q.entropy()[idx1]
#        print(q_phi_entropy)        
        elbo10 = L/self.num_nodes**magic_power * q_phi_entropy.sum()
