...
#                         T_samples.expand(L,self.num_samples).t()))
        #log1pexp_arg = lambda c,R,T: ((2*c).log()-R)/(2*T)
        l1pe = ((cd_clamped*2).log()-R_samples.expand(L,self.num_samples).t())/(2*T_samples.expand(L,self.num_samples).t())
        #p_halfbaked = 
        #
        
        lp_raw = edges*(-log1pexp(l1pe)) + (1-edges)*(log1pexp_(l1pe))
        #lp_clamped = torch.clamp(warn_tensor(lp_raw,'lp_raw'), min=self.epsilon, max=1.-self.epsilon)
